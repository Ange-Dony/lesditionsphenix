...
const { Client } = pg;
const dbUrl = 'postgresql://postgres.sdkvmqvdmmypoclfivvo:Kcx5S5dWzDQJC4dzdrkmmq6byK7Ac7aIybF5oIEQIVY@aws-0-eu-central-1.pooler.supabase.com:6543/postgres';
...
CREATE INDEX IF NOT EXISTS idx_ouvrages_collection ON public.ouvrages(collection_id);
CREATE INDEX IF NOT EXISTS idx_ouvrages_publie ON public.ouvrages(disponibilite);
CREATE INDEX IF NOT EXISTS idx_ressources_publie ON public.ressources(publie);
CREATE INDEX IF NOT EXISTS idx_ressources_filtres ON public.ressources(niveau, matiere, type);
...
DROP POLICY IF EXISTS "Collections visibles par tous" ON public.collections;
...
DROP POLICY IF EXISTS "Ouvrages visibles par tous" ON public.ouvrages;
CREATE POLICY "Ouvrages visibles par tous" ON public.ouvrages FOR SELECT USING (disponibilite = true);
DROP POLICY IF EXISTS "Admins full access ouvrages" ON public.ouvrages;
CREATE POLICY "Admins full access ouvrages" ON public.ouvrages FOR ALL USING (public.is_admin());

DROP POLICY IF EXISTS "Ressources visibles par tous" ON public.ressources;
CREATE POLICY "Ressources visibles par tous" ON public.ressources FOR SELECT USING (publie = true);
DROP POLICY IF EXISTS "Admins full access ressources" ON public.ressources;
CREATE POLICY "Admins full access ressources" ON public.ressources FOR ALL USING (public.is_admin());

DROP POLICY IF EXISTS "Parametres visibles par tous" ON public.parametres_site;
CREATE POLICY "Parametres visibles par tous" ON public.parametres_site FOR SELECT USING (true);
DROP POLICY IF EXISTS "Admins full access parametres" ON public.parametres_site;
CREATE POLICY "Admins full access parametres" ON public.parametres_site FOR ALL USING (public.is_admin());

-- 6. STORAGE BUCKETS (If available)
-- Note: inserting into storage.buckets requires specific permissions, wrapping in a DO block to ignore errors
DO $$
BEGIN
...
-- Try applying policies to storage.objects, ignore if error
DO $$
BEGIN
  ...
  DROP POLICY IF EXISTS "Medias admin write" ON storage.objects;
...
-- 7. DEMO DATA (Using UPSERT-like behavior or ignoring conflicts)
INSERT INTO public.parametres_site (id, telephone_whatsapp, coordonnees, infos_generales) 
VALUES ('p1000000-0000-0000-0000-000000000001', '+33612345678', '10 Rue de la Plume, 75001 Paris', 'Les Éditions Phénix : Renaître par le savoir.')
...
INSERT INTO public.ressources (id, titre, type, niveau, matiere, ouvrage_id, collection_id, format, google_drive_url, publie) VALUES 
  ('r1000000-0000-0000-0000-000000000001', 'Corrigés Chapitre 1-3', 'Corrigé', 'Lycée', 'Mathématiques', 'o2000000-0000-0000-0000-000000000002', 'c2000000-0000-0000-0000-000000000002', 'PDF', 'https://drive.google.com/example-corriges', true),
  ('r2000000-0000-0000-0000-000000000002', 'Fiche de lecture - Cendres du Temps', 'Fiche de cours', 'Lycée', 'Littérature', 'o3000000-0000-0000-0000-000000000003', 'c3000000-0000-0000-0000-000000000003', 'PDF', 'https://drive.google.com/example-fiche', true)
ON CONFLICT DO NOTHING;
        `;
